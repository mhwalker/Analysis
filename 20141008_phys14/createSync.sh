...
condorFile=$SCRIPTDIR/submitSync_${sample}.condor
...
runScript=$SCRIPTDIR/runSync.sh
if [ -e $runScript ]
then
    rm -rf $runScript
fi
...
echo 'root -q -b -l syncMacro.C\(\"$1\",\"$2\",$3\)' >> $runScript
echo "" >> $runScript
...
do
  ...
  base=`echo $line | awk '{split($9,array,"flat_"); split(array[2],array2,"_");split(array2[2],array3,".");print array3[1]}'`
  #ifname=$basedir/output_${base}.root
  ifname=$basedir/flat_${sample}_${base}.root
  mode=0
  ofile=histograms/histo_${sample}_${base}.root
  echo "output = $LOGDIR/\$(Cluster)_sync_${sample}_${base}.out" >> $condorFile
  echo "error = $LOGDIR/\$(Cluster)_sync_${sample}_${base}.err" >> $condorFile
  echo "log = $LOGDIR/\$(Cluster)_sync_${sample}_${base}.log" >> $condorFile
